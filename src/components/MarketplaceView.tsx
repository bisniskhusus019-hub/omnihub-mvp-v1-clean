...
export default function MarketplaceView({ products, onBuy }: MarketplaceViewProps) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/20 bg-cyan-500/10 px-3 py-1 text-[11px] font-black uppercase tracking-widest text-cyan-300 mb-3">
              <SparklesIcon /> Multi-purpose Marketplace
            </div>
            ...
            <p className="text-sm text-slate-400 max-w-3xl leading-6">
              Discover digital products, seller services, storefronts, affiliate-ready offers, and community-promotable listings in one growth network.
            </p>
            <div className="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-2 max-w-3xl">
              {['Buy products', 'Hire services', 'Copy affiliate links', 'Discuss in community'].map((item) => (
                <div key={item} className="rounded-2xl border border-slate-800 bg-slate-950/70 px-3 py-2 text-[11px] font-bold text-slate-300">{item}</div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full lg:w-auto">
            <label className="rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2">
              <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-1"><Globe2 size={11} /> Language</div>
              <select value={language} onChange={(event) => updateLanguage(event.target.value)} className="w-full bg-transparent text-sm text-slate-200 focus:outline-none">
                {supportedLanguages.map((item) => <option key={item.code} value={item.code} className="bg-slate-900 text-slate-100">{item.nativeLabel}</option>)}
              </select>
            </label>

            <label className="rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2">
              <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-1"><Coins size={11} /> Currency</div>
              <select value={currency} onChange={(event) => updateCurrency(event.target.value)} className="w-full bg-transparent text-sm text-slate-200 focus:outline-none">
                {supportedCurrencies.map((item) => <option key={item.code} value={item.code} className="bg-slate-900 text-slate-100">{item.code} — {item.label}</option>)}
              </select>
            </label>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {categories.map((category) => {
            ...
            return (
              <button key={category} onClick={() => setActiveCategory(category)} className={`flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${activeCategory === category ? 'bg-cyan-500 text-slate-900 shadow-lg shadow-cyan-500/20' : 'bg-slate-900 border border-slate-700 text-slate-400 hover:text-white hover:border-slate-600'}`}>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16"><div className="text-slate-600 text-5xl mb-4">¯\_(ツ)_/¯</div><p className="text-slate-400 text-sm">No products found. Try a different search.</p></div>
      ) : (
            ...
            return (
              <div key={product.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-cyan-500/25 hover:shadow-xl hover:shadow-cyan-500/5 transition-all duration-300 group flex flex-col">
                <div className="relative h-44 overflow-hidden flex-shrink-0">
                  ...
                  {eligible && <span className="absolute bottom-3 left-3 inline-flex items-center gap-1 text-[10px] bg-emerald-500/15 backdrop-blur-sm text-emerald-300 px-2 py-1 rounded-lg border border-emerald-500/20 font-bold"><Megaphone size={10} />Affiliate-ready</span>}
                </div>
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <div className="flex items-center gap-1.5 mb-3">
                    {[...Array(5)].map((_, index) => <Star key={index} size={10} className={index < Math.floor(rating) ? 'text-amber-400 fill-amber-400' : 'text-slate-700'} />)}
                    <span className="text-[10px] text-slate-500 ml-0.5">{rating} ({sold} sold)</span>
                  </div>

                  <div className="flex items-center gap-2 pb-3 mb-3 border-b border-slate-800">
                    <img src={getSellerAvatar(product)} alt={getSellerName(product)} className="w-6 h-6 rounded-full object-cover border border-slate-700" />
                    <div className="flex-1 min-w-0"><p className="text-[11px] font-medium text-slate-300 truncate">{getSellerName(product)}</p><p className="text-[10px] text-slate-500">@{getSellerUsername(product)}</p></div>
                    <button onClick={() => copyLink(product, 'product')} className="flex items-center gap-1 text-[10px] text-cyan-400 hover:text-cyan-300 font-medium transition-colors"><Store size={10} />Store</button>
                  </div>

                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <button onClick={() => copyLink(product, 'affiliate')} className="inline-flex items-center justify-center gap-1 rounded-xl border border-slate-700 bg-slate-950 px-2 py-2 text-[11px] font-bold text-slate-300 hover:text-cyan-300 hover:border-cyan-500/40"><Link2 size={12} />{copied === `${product.id}-affiliate` ? 'Copied' : 'Affiliate link'}</button>
                    <button onClick={() => copyLink(product, 'community')} className="inline-flex items-center justify-center gap-1 rounded-xl border border-slate-700 bg-slate-950 px-2 py-2 text-[11px] font-bold text-slate-300 hover:text-cyan-300 hover:border-cyan-500/40"><MessageSquare size={12} />{copied === `${product.id}-community` ? 'Copied' : 'Discuss'}</button>
                  </div>

                  <div className="flex items-center justify-between gap-2">
                    <div><div className="text-base font-bold text-cyan-400">{formattedConvertedPrice}</div><div className="text-[10px] text-slate-500">Base: {formattedSourcePrice}</div></div>
                    <button onClick={() => onBuy(product)} className="flex items-center gap-1.5 bg-cyan-500 hover:bg-cyan-400 active:scale-95 text-slate-900 font-bold text-xs px-4 py-2 rounded-xl transition-all shadow-lg shadow-cyan-500/20"><ShoppingCart size={12} />{translate(language, 'buyNow')}</button>
                  </div>
                </div>
              </div>
            );
