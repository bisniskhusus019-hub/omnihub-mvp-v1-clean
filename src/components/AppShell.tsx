...
const navItems = [
  ...
  { key: 'promotions', label: 'Promotions', icon: Megaphone, access: 'public' },
  { key: 'finance-trust', label: 'Finance & Trust', icon: Wallet, access: 'public' },
...
const moduleKeys = [
  ...
  { key: 'promotions', label: 'Promotions', access: 'public' },
  { key: 'financeTrust', label: 'Finance & Trust', access: 'public' },
...
export default function AppShell({ activeView, setActiveView, modules, toggleModule, isAuthenticated = false, isPlatformOwner = false, onSignInClick, onSignOut, children }: AppShellProps) {
  ...
  const isDisabled = (key: string) => {
    ...
    if (key === 'finance-trust') return modules.financeTrust === false;
    ...
    <div className="h-screen bg-slate-950 text-slate-100 flex overflow-hidden">
      ...
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} fixed top-0 left-0 h-screen w-64 bg-slate-900 border-r border-slate-800 z-40 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 lg:sticky lg:top-0 lg:self-start`}>
        <div className="flex items-center gap-3 px-5 py-5 border-b border-slate-800">
          <div className="w-8 h-8 rounded-lg bg-cyan-500 flex items-center justify-center"><Zap size={16} className="text-slate-900" /></div>
          <div><div className="font-bold text-white text-sm tracking-wide">OmniHub</div><div className="text-[10px] text-cyan-400 font-medium tracking-widest uppercase">AI Marketplace MVP</div></div>
          <button className="ml-auto lg:hidden text-slate-400 hover:text-white" onClick={() => setSidebarOpen(false)}><X size={18} /></button>
        </div>
        <nav className="flex-1 min-h-0 overflow-y-auto py-4">
          ...
          {visibleNavItems.map(({ key, label, icon: Icon, access }) => {
            ...
            const locked = access === 'seller' && !isAuthenticated;
            return <button key={key} onClick={() => handleNav(key, access)} className={`w-full flex items-center gap-3 px-5 py-2.5 text-sm font-medium transition-all relative ${active ? 'text-cyan-400 bg-cyan-500/10 before:absolute before:left-0 before:top-0 before:bottom-0 before:w-0.5 before:bg-cyan-400' : disabled ? 'text-slate-600' : locked ? 'text-slate-500 hover:text-cyan-300 hover:bg-slate-800/50' : 'text-slate-400 hover:text-slate-100 hover:bg-slate-800/50'}`}><Icon size={16} /><span>{label}</span>{locked && <Lock size={11} className="ml-auto text-slate-600" />}{disabled && <span className="ml-auto text-[9px] bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-medium">OFF</span>}{active && <ChevronRight size={12} className="ml-auto" />}</button>;
          })}
        ...
        <div className="px-5 py-4 border-t border-slate-800"><div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-1">Access Mode</div><div className="text-xs text-slate-300 leading-5">{isPlatformOwner ? 'Owner workspace unlocked.' : isAuthenticated ? 'Seller workspace unlocked.' : 'Public browsing enabled. Seller tools are locked.'}</div></div></div>
      </aside>
      <div className="flex-1 flex flex-col h-screen overflow-hidden min-w-0">
        <header className="sticky top-0 z-20 bg-slate-950/90 backdrop-blur-md border-b border-slate-800 px-4 py-3 flex items-center gap-4">
          <button className="lg:hidden text-slate-400 hover:text-white" onClick={() => setSidebarOpen(true)}><Menu size={20} /></button>
          <div className="hidden sm:flex items-center gap-2 text-xs text-slate-400"><Bot size={14} className="text-cyan-400" /><span>Marketplace + Service Requests + Promotions + Finance + Affiliates + Community are public. Seller tools require login.</span></div>
          <div className="flex-1" />
          <button onClick={() => handleNav('notifications', 'public')} className="relative text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-800"><Bell size={18} /><span className="absolute top-0 right-0 w-2 h-2 bg-cyan-400 rounded-full" /></button>
          <div className="relative"><button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-900 border border-slate-800 hover:border-slate-700"><div className="w-6 h-6 rounded-full bg-gradient-to-br from-cyan-400 to-blue-500" /><span className="hidden sm:block text-xs font-medium text-slate-300">{isPlatformOwner ? 'Owner' : isAuthenticated ? 'Seller' : 'Guest'}</span><ChevronDown size={12} className="text-slate-500" /></button>{userMenuOpen && <div className="absolute right-0 mt-2 w-48 bg-slate-900 border border-slate-800 rounded-xl shadow-xl py-2 z-50"><button className="w-full flex items-center gap-2 px-4 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800"><Settings size={14} />Settings</button>{isAuthenticated ? <button onClick={onSignOut} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-400 hover:text-red-300 hover:bg-slate-800"><LogOut size={14} />Sign out</button> : <button onClick={onSignInClick} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-cyan-400 hover:text-cyan-300 hover:bg-slate-800"><LogIn size={14} />Seller login</button>}</div>}</div>
        </header>
        <main className="flex-1 overflow-y-auto bg-slate-950 min-h-0">{children}</main>
      </div>
    </div>
  );
}
