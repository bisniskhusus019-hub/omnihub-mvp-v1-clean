...
function getFileType(product: any) {
  return product.fileType || product.file_type || product.product_type || 'Digital Asset';
}

function getFileSize(product: any) {
  return product.fileSize || product.file_size || 'Secure delivery';
}
        ...
        <div className="relative flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">
              Your purchase has been confirmed and your digital asset is ready for download.
            </p>
            {transaction && (
              <p className="text-xs text-slate-500 mt-1">
                Purchased by <span className="text-slate-300 font-medium">{getBuyer(transaction)}</span> · {getDate(transaction)}
              </p>
            )}
          </div>
        </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-bold text-white mb-2 leading-snug">
                {selectedProduct.title || transaction?.product_title || 'OmniHub Product'}
              </h3>
              <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-slate-400">
                <span className="flex items-center gap-1.5 bg-slate-800 px-2 py-1 rounded-lg">
                  ...
                  {getFileType(selectedProduct)}
                </span>
                <span className="flex items-center gap-1.5 bg-slate-800 px-2 py-1 rounded-lg">
                  <Package size={11} className="text-slate-500" />
                  {getFileSize(selectedProduct)}
                </span>
                {transaction && (
                  <span className="flex items-center gap-1.5 bg-slate-800 px-2 py-1 rounded-lg">
                    <User size={11} className="text-slate-500" />
                    {getBuyer(transaction)}
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              This digital asset is protected by OmniHub Token-Bound Access technology. Your unique secure token ({secureToken.substring(0, 12)}...) is bound to your transaction identity. Downloads are limited and expire after 72 hours.
            </p>
          </div>
        </div>
      </div>
